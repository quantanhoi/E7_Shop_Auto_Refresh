...
def buy(bookmark):
    ...
    while((pos == None) and (time.time()< (buy_start + timeout))):
        if (bookmark == 'covenant'):
            pos = pyautogui.locateOnScreen('covenant.png',confidence=0.7)
        else:
            pos = pyautogui.locateOnScreen('mystic.png',confidence=0.7)
            
    pos_point = pyautogui.center(pos)
    #click buy
    click(pos_point[0]+800+rand_x, pos_point[1]+40+rand_y)
    click(pos_point[0]+800+rand_x, pos_point[1]+40+rand_y)
    time.sleep(random.uniform(1, 2)) #wait for confirm button
    ...
    while(time.time() < (timeout_start + timeout)):
        ...
        print("Located", bookmark)
        if (Buy_button_pos != None):
            print("Confirming Purchase ", bookmark)
